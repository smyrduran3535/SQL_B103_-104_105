...
--cozulemek cunku ted verginosu 105 olan veri yok.

--SORU 3: child tablosuna ted_vergino null olan veri girişi yapınız
insert into child values(null,2000,'Araba','Burak');
--child tabloya null deger girebiliyoruz

--SORU 4: .parent tablosundaki vergi_no 101 olan veriyi siliniz.
--silemeyz f.k ile pr.k birbirine baglı oldugu icin silinmez. 
--bu yuzden once child tablodaki veri silinmeli
delete from parent where vergi_no=101; --error verir
...
delete from parent where vergi_no=101; -- childden silindigi icin silinir

-SORU 7: .parent tablosunu siliniz.***once child silinecek
drop table  child;
drop table parent; --silinir

--ON DELETE CASCATE:
--on delete cascade komutu ile child verileri silinmeden parent silinebilir.
--ancak parent ile child in verileri silinir.
---child tablo dursun parent silinsin istiycez bunun icin
--on delete cascat sayesinde bunu yapabiliyoruz
...
--Parent tablonun kendisini silmek isteyince CASCADE diye belirtmemiz gerekiyor
--cascade yazmaliyız child ı silmeden parentı silebiliyoruz
--on delete cascate sayesinde child tablo silinmeden parent tablo silinebildi
----on delete cascade sayesinde child tablol silinmeden parent tablo silinebildi***TABLO*** CHİLD SİLİNMEZ
--fakat bu durumda child tablodaki veriler de siliniyor***VERİLER*** CHİLD DEKİ VERİLER SİLİNEBİLİR

--SORU3: child2 tablosunu siliniz;
drop table child2; --cascate kullanmadan sildik bunu
...
update toptancilar
set sirket_ismi='VivaBook' where vergi_no=202;
